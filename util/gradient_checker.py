...
(copied from tensorflow.python.ops.gradient_checker; modified to support
using feed_dict values provided by user)
"""
...
def _compute_theoretical_jacobian(x, x_shape, x_data, dy, dy_shape, dx, feed_dict,
                                  prep_fn=None):
  ...
  prep_fn = (lambda: None) if prep_fn is None else prep_fn
  ...
  feed_copy.update({x: x_data, dy: dy_data})
  ...
  for col in range(dy_size):
    ...
    if isinstance(dx, ops.IndexedSlices):
      backprop_indices, backprop_values = sess.run(
          [dx.indices, dx.values], feed_dict=feed_copy)
      ...
      backprop = sess.run(dx, feed_dict=feed_copy)
...
def _compute_numeric_jacobian(x, x_shape, x_data, y, y_shape, delta, feed_dict,
  ...
  for row in range(x_size):
    ...
    feed_copy[x] = x_pos
    prep_fn()
    y_pos = y.eval(feed_dict=feed_copy)

    x_neg.ravel().view(x_dtype)[row] -= delta

    feed_copy[x] = x_neg
    prep_fn()
    y_neg = y.eval(feed_dict=feed_copy)
...
def _compute_gradient_list(x,
  ...
  for xi, x_shapei, dxi, dyi, x_init_valuei in zip(x, x_shape, dx, dy, x_init_value):
      ...
      ret.append(_compute_gradient(xi, x_shapei, dxi, y, y_shape, dyi, x_init_valuei,
                                   delta, feed_dict, prep_fn=prep_fn, limit=limit))
  return ret
...
def compute_gradient(x,
  ...
  if isinstance(x, list):
    return _compute_gradient_list(x, x_shape, y, y_shape, x_init_value, delta,
                                  init_targets, feed_dict, prep_fn=prep_fn,
                                  limit=limit)
  else:
    if init_targets is not None:
      assert isinstance(init_targets, (list, tuple))
    ...
    ret = _compute_gradient(x, x_shape, dx, y, y_shape, dy, x_init_value, delta, feed_dict, prep_fn=prep_fn, limit=limit)
    return ret
...
def compute_gradient_error(x,
  ...
  grad = compute_gradient(x, x_shape, y, y_shape, x_init_value, delta,
                          init_targets, feed_dict, prep_fn, limit)
