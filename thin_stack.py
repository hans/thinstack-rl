...
class ThinStack(object):
    ...
    def _update_stack(self, t, shift_value, reduce_value, transitions_t):
        ...
        queue_next = tf.scatter_update(self.queue, queue_idxs,
                                       tf.fill((self.batch_size,), t),
                                       name="queue_update")
    ...
    def _lookup(self, t):
        ...
        stack1 = tf.gather(self.stack, tf.maximum(0, stack1_ptrs), name="stack1_fetch")

        queue_ptrs = (self.cursors - 1) * self.batch_size + self.batch_range
        stack2_ptrs = tf.gather(self.queue, tf.maximum(0, queue_ptrs)) * self.batch_size + self.batch_range
        stack2 = tf.gather(self.stack, stack2_ptrs, name="stack2_fetch")
...
def main():
    ...
    with tf.variable_scope("m", initializer=util.HeKaimingInitializer()):
        ...
        ts = ThinStack(compose_fn, tracking_fn, transition_fn, batch_size,
                       vocab_size, num_timesteps, model_dim, embedding_dim,
                       tracking_dim, tf.constant(True), embedding_initializer=integer_embedding_initializer)
    ...
    X, transitions, y, lengths = util.data.BucketToArrays(data, FakeDataManager())
    buff = X[:, 0, :].T
